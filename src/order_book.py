...
class Trade:
    ...
    buy_order_id: int
    sell_order_id: int
    buyer: int  # owner handle (connection_id) that gets BOUGHT
    seller: int  # owner handle (connection_id) that gets SOLD


class Order:
    def __init__(self, order_id, owner, type, instrument, price, quantity):
        self.order_id = order_id
        self.owner = owner  # connection_id; book ignores it, just carries it
...
class OrderBook:
    def __init__(self):
        ...
        self._next_id = 0  # book owns the counter; assigned only to accepted orders

    def validate_order(self, type, instrument, price, quantity):
    ...
    def process_order(self, new_order):
        ...
        if new_order.quantity > 0:
            self.order_dicts[new_order.type][new_order.instrument].setdefault(
                new_order.price, []
            ).append(new_order)
    ...
    def cancel(self, owner, order_id):
        ...
        if order is None:
            return False, "No such active order"
        if order.owner != owner:
            return False, "You cannot cancel others' orders"
